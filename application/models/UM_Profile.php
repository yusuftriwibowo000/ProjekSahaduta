...
class UM_Profile extends CI_Model
{
	...
	public function get_riwayat($no)
	{	
		$query = $this->db->query("SELECT * FROM tb_pemesanan LEFT JOIN tb_pasien USING(no_rm) LEFT JOIN tb_pegawai USING(id_pegawai) WHERE no_rm = $no");
		return $query->result();
		...
		// $this->db->where('tb_pemesanan.no_rm = 22' );
