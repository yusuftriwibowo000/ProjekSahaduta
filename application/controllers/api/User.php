...
class User extends REST_Controller
{
	...
	function index_get($no_rm=null){	
		...
		if ($no_rm==null||$no_rm==0) {
			#Call methode get_all from m_users model
			$users=$this->m_users->get_all($no_rm);
		...
		if ($no_rm!=null&&$no_rm!=0) {
			...
			$users=$this->m_users->get_by_no_rm($no_rm);
		}
	...
	function index_post(){
		...
		#
		$user_data = array(	'password' =>md5($this->post('password')),
							'nama_pasien' => $this->post('nama_pasien') ,
							'tgl_lahir' => $this->post('tgl_lahir') ,
							'alamat' => $this->post('alamat') ,
							'nama_kk' => $this->post('nama_kk') , 
							'id_agama' => $this->post('id_agama') ,
